package eu.fluffici.furraid.events.user;
...
public class MemberMessageListener extends ListenerAdapter {
    ...
    @SneakyThrows
    public MemberMessageListener(FurRaidDB instance) {
    ...
    public void onMessageReceived(@NonNull MessageReceivedEvent event) {
        ...
        TicketBuilder ticket = this.instance.getGameServiceManager()
                .fetchFTicket(event.getChannel().getId(), event.getGuild().getId());

        if (ticket != null) {
           try {
               this.instance.getTicketManager().addTicketMessage(event.getMessage(), ticket.getTicketId());
           } catch (Exception e) {
               e.printStackTrace();
           }
        }
