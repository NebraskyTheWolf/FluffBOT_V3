...
@CommandHandle
public class CommandCuddles extends Command {
    public CommandCuddles() {
        super("cuddles", "Cuddly OwO", CommandCategory.FUN);
    ...
    public void execute(CommandInteraction interaction) {
        ...
        JsonArray images = this.getImages("cuddles.json");
        ...
        if (images.size() <= 0) {
            ...
            interaction.replyEmbeds(
                    this.getEmbed()
                            .image(String.format("%s se pomazlil/a s %s", interaction.getUser().getEffectiveName(), target.getEffectiveName()), "", imageUrl)
                            .build()
