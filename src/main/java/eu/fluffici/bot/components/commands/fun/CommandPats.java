...
@CommandHandle
public class CommandPats extends Command {

    public CommandPats() {
        super("pats", "Give someone a headpat to show your affection or playful spirit! owo", CommandCategory.FUN);
    ...
    public void execute(CommandInteraction interaction) {
       ...
       JsonArray images = this.getImages("pats.json");
       ...
       if (images.size() <= 0) {
           ...
           interaction.replyEmbeds(
                   this.getEmbed()
                           .image(String.format("%s se pohladil/a s %s", interaction.getUser().getEffectiveName(), target.getEffectiveName()), "", imageUrl)
                           .build()
