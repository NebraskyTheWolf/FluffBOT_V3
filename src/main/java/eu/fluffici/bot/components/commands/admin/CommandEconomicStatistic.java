...
@CommandHandle
public class CommandEconomicStatistic extends Command {
    public CommandEconomicStatistic() {
        super("economic-statistic", "See the weekly statistics of the economy.", CommandCategory.ADMINISTRATOR);
    ...
    public void execute(CommandInteraction interaction) {
        Pair<Boolean, Pair<String, FileUpload>> personalStatistics = FluffBOT.getInstance().getDefaultCharts().getEconomicChart(
                FluffBOT.getInstance().getGameServiceManager().getAllEconomyRecord().stream().distinct().toList(),
