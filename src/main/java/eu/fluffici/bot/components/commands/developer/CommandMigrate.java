...
/*
---------------------------------------------------------------------------------
File Name : CommandProfileMigrate.java
...
public class CommandMigrate extends Command {
    public CommandMigrate() {
        super("migrate", "Bot uptime.", CommandCategory.DEVELOPER);
    ...
    public void execute(CommandInteraction interaction) {
        ...
        GuildAddListener.handleMigration(
                interaction.getGuild(),
                interaction.getMember()
        );

        interaction.replyEmbeds(this.buildSuccess("Migration completed successfully!")).setEphemeral(true).queue();
    }
}
