...
public class CommandBoop extends Command {
    ...
    public CommandBoop() {
        super("boop", "Give someone a friendly boop to show your affection or playful spirit! owo", CommandCategory.FUN);
    ...
    public void execute(CommandInteraction interaction) {
       ...
       JsonArray images = this.getImages("boops.json");
       ...
       if (images.size() <= 0) {
           ...
           interaction.replyEmbeds(
                   this.getEmbed()
                           .image(String.format("%s boopnul/a s %s", interaction.getUser().getEffectiveName(), target.getEffectiveName()), "", imageUrl)
                           .build()
